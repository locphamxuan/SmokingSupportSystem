...
const NotificationsPage = () => {
  ...
  useEffect(() => {
    async function fetchNotifications() {
      try {
        ...
        let publicNoti = [];
        if (publicData.reward)
          publicNoti = publicNoti.concat(
            publicData.reward.map((n) => ({ ...n, type: "reward" })),
          );
        if (publicData.daily)
          publicNoti = publicNoti.concat(
            publicData.daily.map((n) => ({ ...n, type: "daily" })),
          );
        if (publicData.weekly)
          publicNoti = publicNoti.concat(
            publicData.weekly.map((n) => ({ ...n, type: "weekly" })),
          );
        if (publicData.motivation)
          publicNoti = publicNoti.concat(
            publicData.motivation.map((n) => ({ ...n, type: "motivation" })),
          );
        // Gán id cho thông báo chung nếu chưa có
        publicNoti = publicNoti.map((n, idx) => ({
          ...n,
          id: n.Id || n.id || `public-${n.type}-${idx}`,
        }));
        // Gộp tất cả
  ...
  if (loading)
    return <div className="text-center py-5">Đang tải thông báo...</div>;
