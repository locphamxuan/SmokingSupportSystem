...
const CoachMemberProgressPage = () => {
  ...
  useEffect(() => {
    ...
    if (memberId) {
      ...
      setError("");
      axios
        .get(`http://localhost:5000/api/coach/member/${memberId}/progress`, {
      ...
      setError("");
      axios
        .get("http://localhost:5000/api/booking/accepted", {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => setMembers(res.data.bookings || []))
        .catch(() =>
          setError("Không thể tải danh sách thành viên đã nhận lịch."),
        )
        .finally(() => setLoading(false));
    }
  }, [memberId]);
  ...
  if (loading)
    return (
      <div className="container py-5 text-center">Đang tải dữ liệu...</div>
    );
  if (error)
    return (
  ...
  if (memberId) {
    ...
    const chartData = {
      labels: progressHistory.map((log) =>
    ...
    return (
      <div className="container py-4 mt-5">
        ...
        <h3 className="mb-4">
          Tiến trình của thành viên {memberInfo?.username || memberId}
        </h3>
        ...
        {smokingProfile && (
          <div className="mb-4">
            ...
            <ul>
              <li>
                Số điếu/ngày: <b>{smokingProfile.cigarettesPerDay}</b>
              </li>
              <li>
                Loại thuốc: <b>{smokingProfile.cigaretteType}</b>
              </li>
              <li>
                Tần suất hút: <b>{smokingProfile.smokingFrequency}</b>
              </li>
              <li>
                Lý do cai: <b>{smokingProfile.quitReason}</b>
              </li>
              <li>
                Tình trạng sức khỏe: <b>{smokingProfile.healthStatus}</b>
              </li>
            </ul>
          </div>
        )}
        ...
        {coachQuitPlan && (
          <div className="mb-4">
            ...
            <ul>
              <li>
                Ngày bắt đầu: <b>{coachQuitPlan.startDate}</b>
              </li>
              <li>
                Ngày mục tiêu: <b>{coachQuitPlan.targetDate}</b>
              </li>
              <li>
                Số điếu ban đầu: <b>{coachQuitPlan.initialCigarettes}</b>
              </li>
              <li>
                Giảm mỗi ngày: <b>{coachQuitPlan.dailyReduction}</b>
              </li>
              <li>
                Mô tả: <b>{coachQuitPlan.planDetail}</b>
              </li>
            </ul>
          </div>
        )}
        ...
        {systemQuitPlan && (
          <div className="mb-4">
            ...
            <ul>
              <li>
                Ngày bắt đầu: <b>{systemQuitPlan.startDate}</b>
              </li>
              <li>
                Ngày mục tiêu: <b>{systemQuitPlan.targetDate}</b>
              </li>
              <li>
                Số điếu ban đầu: <b>{systemQuitPlan.initialCigarettes}</b>
              </li>
              <li>
                Giảm mỗi ngày: <b>{systemQuitPlan.dailyReduction}</b>
              </li>
              <li>
                Mô tả: <b>{systemQuitPlan.planDetail}</b>
              </li>
            </ul>
          </div>
        )}
        ...
        <div className="mb-4">
          ...
          {progressHistory.length === 0 ? (
            <div className="alert alert-info">
              Chưa có nhật ký tiến trình nào.
            </div>
          ) : (
  ...
  const uniqueMembers = Array.from(
    new Map(members.map((m) => [m.MemberId, m])).values(),
  );
  ...
  return (
    <div className="container py-4 mt-5">
      ...
      {uniqueMembers.length === 0 ? (
        <div className="alert alert-info">
          Bạn chưa nhận lịch cho thành viên nào.
        </div>
      ) : (
          ...
          {uniqueMembers.map((member) => (
            ...
            >
              ...
              <button
                ...
                onClick={() =>
                  navigate(`/coach/member/${member.MemberId}/progress`)
                }
              >
                Xem tiến trình
              </button>
