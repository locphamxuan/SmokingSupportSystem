...
const SubscriptionPlans = () => {
  ...
  const handlePaymentSuccess = async () => {
    try {
      ...
      const response = await axios.put(
        "http://localhost:5000/api/auth/upgrade-member",
        {},
        {
      ...
      if (response.data && response.data.user) {
        ...
        setUser(updatedUser);
        console.log(
          "[SubscriptionPlans] User state after successful upgrade:",
          updatedUser,
        ); // DEBUG
      ...
      if (error.response?.status === 401) {
        ...
        setError(
          error.response.data.message ||
            "Không thể nâng cấp tài khoản. Vui lòng thử lại sau.",
        );
      } else {
  ...
  const isPremiumMember =
    user && (user.role === "memberVip" || user.isMemberVip === true);
  ...
  return (
    ...
    >
      <section
        className="subscription-plans-section"
      ...
      >
        ...
        >
          ...
          {user && (user.role === "memberVip" || user.isMemberVip === true) ? (
            <>
              <div className="card premium-status-card text-center mb-4">
                ...
                <h2>Bạn đã đăng ký gói thành viên VIP</h2>
                <p>
                  Cảm ơn bạn đã tin tưởng và sử dụng gói thành viên VIP của
                  chúng tôi.
                </p>
                <button
            ...
            <>
              <h2>Chọn gói thành viên phù hợp với bạn</h2>
              <p>
                Nâng cấp lên <b>thành viên VIP</b> để trải nghiệm đầy đủ tính
                năng.
              </p>
              <div className="row justify-content-center">
                {packages.map((pkg) => (
                  <div key={pkg.id} className="col-12 col-md-6 col-lg-5 mb-4">
                    <div className="card h-100">
                      <div className="card-body">
                        ...
                        <p>
                          {pkg.name === "Gói Premium"
                            ? "Truy cập đầy đủ tất cả tính năng VIP"
                            : pkg.description}
                        </p>
                        <hr className="my-3" />
                        <ul className="list-unstyled mb-0">
                        ...
                        {pkg.price === 0 ? (
                          ...
                          <a
                            href="/login"
                            className="btn btn-lg btn-primary mt-4 rounded-pill"
                          >
          ...
          {success && (
            <div
              className="alert alert-success alert-dismissible fade show text-center mt-4"
              role="alert"
            >
              {success}
              <button
