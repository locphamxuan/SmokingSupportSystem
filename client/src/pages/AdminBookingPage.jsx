...
const AdminBookingPage = () => {
  ...
  const fetchCoaches = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5000/api/auth/coaches",
        {
  ...
  const filterBookings = useCallback(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (booking) =>
          booking.MemberName?.toLowerCase().includes(
    ...
    if (dateFilter === "today") {
      ...
      filtered = filtered.filter(
        (booking) => new Date(booking.SlotDate).toDateString() === today,
      );
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5000/api/booking/admin/paid-bookings",
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const confirmAssignCoach = async () => {
    ...
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/booking/admin/assign-coach",
        {
      ...
      setError(
        err.response?.data?.message || "Không thể phân công huấn luyện viên",
      );
      ...
      <div className="admin-container">
        ...
        <div className="card">
          <div className="card-header">
            <h5 className="mb-0">
              Danh sách lịch hẹn đã thanh toán ({filteredBookings.length})
            </h5>
          </div>
                      ...
                      <div className="card bg-light">
                        <div className="card-body">
                          ...
                          <p className="mb-0">
                            ...
                            {getSlotLabel(selectedBooking.Slot)}
                          </p>
                        </div>
                      </div>
                    </div>
