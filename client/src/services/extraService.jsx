...
export async function addDailyLog({
  ...
  if (!res.ok) {
    ...
    try {
      ...
      console.error(
        "❌ [addDailyLog] Failed to parse error response as JSON:",
        e,
      );
...
// Save User Selected Suggested Quit Plan with Automatic Stage Creation
export async function saveUserSuggestedQuitPlan({
  suggestedPlanId,
  startDate,
  targetDate,
}) {
  const res = await fetch(`${API_BASE}/user-suggested-quit-plan`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${getToken()}`,
  ...
  if (!res.ok) {
    ...
    throw new Error(errorData.message || "Failed to save suggested quit plan");
  }
